...
/**
 * Set the default options for donut
 */

defaultPlotOptions.donut = merge(defaultSeriesOptions, {
	//dragType: '', // n/a
	borderColor: '#FFFFFF',
	borderWidth: 1,
	center: ['50%', '50%'],
...
/**
 * The Donut series class
 */
var DonutSeries = extendClass(Series, {
	type: 'donut',
	isCartesian: false,
	pointClass: PiePoint,
	pointAttrToOptions: { // mapping between SVG attributes and the corresponding options
	...
	/**
	 * Donuts have one color each point
	 */
	getColor: function () {
		// record first color for use in setData
		this.initialColor = this.chart.counters.color;
	...
	/**
	 * Do translation for Donut slices
	 */
	translate: function () {
		this.generatePoints();
		...
		each(points, function (point) {
			...
			point.labelPos = [
				positions[0] + radiusX + mathCos(angle) * labelDistance, // first break of connector
				positions[1] + radiusY + mathSin(angle) * labelDistance, // a/a
				positions[0] + radiusX + mathCos(angle) * connectorOffset, // second break, right outside Donut
				positions[1] + radiusY + mathSin(angle) * connectorOffset, // a/a
				positions[0] + radiusX, // landing point for connector
				positions[1] + radiusY, // a/a
				labelDistance < 0 ? // alignment
	...
	/**
	 * Override the base drawDataLabels method by Donut specific functionality
	 */
	drawDataLabels: function () {
		var series = this,
			data = series.data,
		 ...
		 * of the Donut to detect overlapping labels.
		 */
		while (i--) {
			...
			// Only do anti-collision when we are outside the Donut and have connectors (#856)
			if (distanceOption > 0) {
	...
	/**
	 * Donuts don't have point marker symbols
	 */
	getSymbol: function () {}

});
seriesTypes.donut = DonutSeries;
